...
import PCComponent26 from '../components/PCComponent26';
...
function Formulario26() {
    ...
    return <div>
       ...
       {data.filter(item => componentType ? item.type === componentType ? item : null : item).map(item => <Grid item xs={3}>
        <PCComponent26 title={item.name} image={item.image} fabricante={item.fabricante}/>
       </Grid>)}
       </Grid>
    </div>
}

export default Formulario26