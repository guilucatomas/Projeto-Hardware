...
import PCComponent29 from '../components/PCComponent29';
...
function Formulario29() {
    ...
    return <div>
       ...
       {data.filter(item => componentType ? item.type === componentType ? item : null : item).map(item => <Grid item xs={3}>
        <PCComponent29 title={item.name} image={item.image} fabricante={item.fabricante}/>
       </Grid>)}
       </Grid>
    </div>
}

export default Formulario29