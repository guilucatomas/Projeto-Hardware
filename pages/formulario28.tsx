...
import PCComponent28 from '../components/PCComponent28';
...
function Formulario28() {
    ...
    return <div>
       ...
       {data.filter(item => componentType ? item.type === componentType ? item : null : item).map(item => <Grid item xs={3}>
        <PCComponent28 title={item.name} image={item.image} fabricante={item.fabricante}/>
       </Grid>)}
       </Grid>
    </div>
}

export default Formulario28