...
import PCComponent24 from '../components/PCComponent24';
...
function Formulario24() {
    ...
    return <div>
       ...
       {data.filter(item => componentType ? item.type === componentType ? item : null : item).map(item => <Grid item xs={3}>
        <PCComponent24 title={item.name} image={item.image} fabricante={item.fabricante}/>
       </Grid>)}
       </Grid>
    </div>
}

export default Formulario24